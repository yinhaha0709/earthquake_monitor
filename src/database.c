...
void mysqldb_query_csv(MYSQL *mysql, char *content, char *table_name, char *name, char *value)
{
    ...
    for (j = i - 1; j >= 0; j--)
    {
        fprintf(fp_csv, "%f\t%f\t%f\t%f\t%f\t%f\n", csv_save_array[j][1], csv_save_array[j][2], csv_save_array[j][3], csv_save_array[j][4], csv_save_array[j][5], csv_save_array[j][6]);
    }
    fclose(fp_csv);
